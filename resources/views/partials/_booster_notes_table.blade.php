        ...
        @else
            @foreach ($students as $student)
                <tr>
                    ...
                    <td class="align-middle bg-transparent border-bottom">
                        @php
                            $studentNote = $notes->firstWhere('booster_student_id', $student->id);
                        @endphp
                        <input
                            type="number"
                            class="note-input"
                    ...
                    <td class="text-center d-flex justify-content-evenly align-items-center align-middle bg-transparent border-bottom">
                        <form role="form" class="form" id="note-form">
                            @csrf
                            <input type="hidden" id="booster_note_id" name="booster_note_id" value="{{ isset($studentNote) ? $studentNote->id : '' }}">
                            <input type="hidden" name="booster_student_id" value="{{ $student->id }}">
                            <input type="hidden" name="booster_matiere_id" value="{{ $matiereFilter }}">
                            <input
                                type="hidden"
                                name="evaluation_id"
                                @php
                        ...
                        <button
                            type="button"
                            id="edit-boosternote-button"
                            data-bs-toggle="modal"
                            data-bs-target="#update-boosternote-modal-{{ isset($studentNote) ? $studentNote->id : '' }}"
                        ...
                        @if(isset($studentNote))
                            <div class="modal fade" id="confirmDelete-{{ isset($studentNote) ? $studentNote->id : '' }}" tabindex="-1" aria-labelledby="exampleModalLabel">
                                <div class="modal-dialog">
                                    <form role="form" class="form" method="POST" action="{{ route('booster.notesDestroy', $studentNote->id) }}">
                                        @csrf
                                        @method('DELETE')
                                        <div class="modal-content">
                                            ...
                                            <div class="modal-body text-wrap text-justify">
                                                Voulez-vous vraiment supprimée la note de {{ $student->student->name }}
                                                {{ $student->student->surname }} en {{ $studentNote->matiere->matiere->libelleMatiere }} de
                            ...
                            <div class="modal fade" data-student-id="{{ $student->id }}" id="update-boosternote-modal-{{ isset($studentNote) ? $studentNote->id : '' }}" style="z-index: 30000" tabindex="-1" aria-labelledby="exampleModalLabel">
                                <div class="modal-dialog modal-dialog-centered">
                                    <form enctype="multipart/form-data" role="form" id="updateBoosterNoteForm" class="form row">
                                        @csrf
                                        <input type="hidden" name="boosterNoteId" id="boosterNoteId" value="{{ $studentNote->id }}">
                                        <div class="modal-content p-0">
                                            <div class="modal-header bg-success">
                                                <div class="modal-title row">
                                                    <div class="col-12">
                                                        <h5 class="text-white text-justify text-wrap">
                                                            Vous modifier la note de {{ $student->student->name }} {{ $student->student->surname }}
                                                            en {{ $studentNote->matiere->matiere->libelleMatiere }} pour le compte de
                                            ...
                                            <div class="modal-footer flex-row-reverse">
                                                ...
                                                <button type="button" id="submit-udpate-boosternote-form-button" class="spinner-submit-update-boosternote-form-button btn-outline-success btn btn-lg">
                                                    <span class="spinner-border spinner-border-sm d-none" role="status"></span>
                                                    Mettre à jour
                                                </button>
                                            </div>
